...
void ESPAsync_WiFiManager::setupConfigPortal()
{
  ...
  server->on("/lamp",     std::bind(&ESPAsync_WiFiManager::handleLamp,        this, std::placeholders::_1)).setFilter(ON_AP_FILTER);
  ...
  server->on("/domo", [] (AsyncWebServerRequest *request) {  
    if (request->hasParam( "LampData" )){
...
void ESPAsync_WiFiManager::handleLamp(AsyncWebServerRequest *request)
{
  String page = FPSTR(WM_HTTP_HEAD_START);
  ...
  LOGDEBUG(F("Sent Lamp Controller page"));
}
