...
// Lamp Controller Model
const char BOOTSTRAP_HTTP_STYLE[] PROGMEM = "<link href='https://cdn.jsdelivr.net/npm/bootstrap@5.2.0-beta1/dist/css/bootstrap.min.css' rel='stylesheet'>";
const char JQUERY_HTTP_SCRIPT[] PROGMEM = "<script src='https://ajax.googleapis.com/ajax/libs/jquery/3.4.1/jquery.min.js'></script>";
const char JSON2HTML_HTTP_SCRIPT[] PROGMEM = "<script src='https://cdnjs.cloudflare.com/ajax/libs/json2html/2.1.0/json2html.min.js'></script>";
const char BOOTSTRAP_HTTP_SCRIPT[] PROGMEM = "<script src='https://cdn.jsdelivr.net/npm/bootstrap@5.2.0-beta1/dist/js/bootstrap.bundle.min.js'></script>";
const char LAMPPAGE_HTTP_SCRIPT[] PROGMEM = "<script src='https://indooruwb.s3.eu-west-3.amazonaws.com/anchorlist_loader.js'></script>";
const char LAMPPAGE_STYLE[] PROGMEM = "<style>.de {display: grid;gap: 1rem;grid-template-columns: repeat(auto-fill, minmax(min(100%, 10rem), 1fr));} button,#sm {padding: 5px 20px;margin: 20px;} #cs {margin: 20px;}</style>";
const char LAMPPAGE_BODY[] PROGMEM = "<body><div class='de'><div id='lm'></div><div id='cs'><input id='cp' type='color' value='#00ff00' list='ps'></div><div id='sliders'>Delay<input id='ds' type='range' min='0' max='2000' value='50'><div id='dl'>50</div>Brightness<input id='bs' type='range' min='0' max='100' value='50'><div id='bl'>50</div></div><div><button id='sb'> Send </button></div></div><div><button id='wo'> Wifi OFF </button></div></body>";
const char LAMPPAGE_SCRIPT[] PROGMEM = "<script>let lm=[['Light ON','COLOR'],['Light OFF','OFF'],['Rainbow','RAINBOW'],['Fade','FADE'],['Blink','BLINK'],['Landing','LANDING'],['UpLanding','UPLANDING'],['DownLanding','DOWNLANDING']];let colorInput=domget('cp');let modeInput=domget('lm');window.addEventListener('input',()=>{domlabelvalue('dl','ds');domlabelvalue('bl','bs')});window.addEventListener('load',function(event){const defaultColors=['#00ffff','#00ff00','#0000ff','#ffff00','#ff00ff','#ff0000'];optionConstructor(defaultColors,'datalist','ps','cs');optionConstructor(lm,'select','sm','lm');domget('sb').addEventListener('click',domoData);domget('wo').addEventListener('click',wifioff)});function optionConstructor(optionlist,elementype,id,into){const optionContainer=document.createElement(elementype);optionContainer.setAttribute('id',id);optionlist.forEach((el)=>{const optionelement=document.createElement('option');let optionlabel=el;if(typeof(el)=='object'){optionelement.setAttribute('value',el[1]);optionlabel=el[0]}optionelement.innerHTML=optionlabel;optionContainer.appendChild(optionelement)});domget(into).appendChild(optionContainer)}function domlabelvalue(id,label){domget(id).innerHTML=domvalue(label)}function domvalue(dom){const v=domget(dom).value;if(parseInt(v)>0){return Number(v)}return v}function domget(dom){return document.getElementById(dom)}function RGBColor(){const cv=colorInput.value;function fc(v,b,e){return parseInt('0x'+cv.slice(b,e))};const r=fc(cv,1,3);const g=fc(cv,3,5);const b=fc(cv,5,7);return{r,g,b}}function domoData(){let RGBColour=RGBColor();let lampmode=domvalue('sm');let brightness_value=domvalue('bs');let delay=domvalue('ds');let dataJSON={effect:lampmode,latency:delay,brightness:brightness_value,RGBColor:[RGBColour.r,RGBColour.g,RGBColour.b]};console.log(dataJSON);var xhr=new XMLHttpRequest();xhr.open('GET','domo?LampData='+JSON.stringify(dataJSON),true);xhr.send()}function wifioff(){}</script>";
//////
...
const char WM_HTTP_PORTAL_OPTIONS[] PROGMEM = "<form action='/wifi' method='get'><button class='btn'>Connect to Wifi</button></form><br/><form action='/lamp' method='get'><button class='btn'>Lamp</button></form><br/><form action='/i' method='get'><button class='btn'>Information</button></form><br/><form action='/close' method='get'><button class='btn'>Exit Portal</button></form><br/>";
...
#define RFC952_HOSTNAME_MAXLEN      24
    ...
    void          handleLamp(AsyncWebServerRequest *request);
